...
The dashboard also has adherence views (participant×day heatmap, per-participant sparklines, enrollment curve) backed by a **sharded** participant-daily rollup (`STUDY#<study>#DAILY#<shard>`) + a `FIRST#` first-seen pointer. `SHARDS` is load-bearing across the writer, `libs/metadata_index_reader.py`, and `cluster_management/cdk/backfill_participant_daily.py` (persisted to a `CONFIG` item; the reader asserts it). Populate on an existing deployment with `make metadata-index-backfill APPLY=true` (non-destructive: pauses ingestion, drains by waiting, then `SET`s from existing rollups).
