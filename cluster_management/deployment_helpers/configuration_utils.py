...
def create_finalized_configuration(eb_environment_name):
    ...
    if os.path.exists(finalized_cred_path):
        # On a re-run after a partial failure the file may already exist with valid
        # credentials. Reuse it rather than orphaning the S3 bucket and IAM user.
        existing = get_finalized_settings_variables(eb_environment_name)
        required = {'DOMAIN_NAME', 'RDS_USERNAME', 'RDS_PASSWORD', 'RDS_DB_NAME',
                    'RDS_HOSTNAME', 'FLASK_SECRET_KEY', 'S3_BUCKET',
                    'BEIWE_SERVER_AWS_ACCESS_KEY_ID', 'BEIWE_SERVER_AWS_SECRET_ACCESS_KEY',
                    'SYSADMIN_EMAILS'}
        if required.issubset(existing.keys()):
            log.info("Reusing existing finalized configuration from %s" % finalized_cred_path)
            return existing
        log.error("Finalized configuration file at %s is incomplete. Delete it and re-run." % finalized_cred_path)
        EXIT(1)
