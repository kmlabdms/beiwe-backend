...
def construct_eb_environment_variables(eb_environment_name):
    ...
    # Pin the EB environment to the Beiwe VPC. Without this, EB uses the account
    # default VPC, which won't match the RDS instance or processing servers.
    vpc_id = global_config["VPC_ID"]
    subnets = create_ec2_client().describe_subnets(
        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}]
    )["Subnets"]
    if not subnets:
        raise RuntimeError(f"No subnets found in VPC {vpc_id}. Cannot create EB environment.")
    subnet_list = ",".join(s["SubnetId"] for s in subnets)
    configuration.extend([
        {"Namespace": "aws:ec2:vpc", "OptionName": "VPCId",                    "Value": vpc_id},
        {"Namespace": "aws:ec2:vpc", "OptionName": "Subnets",                  "Value": subnet_list},
        {"Namespace": "aws:ec2:vpc", "OptionName": "ELBSubnets",               "Value": subnet_list},
...
def get_environments_list():
    environments = create_eb_client().describe_environments(IncludeDeleted=False)['Environments']
...
def _sec_grp_id(identifier: str) -> str:
    """Accept a security group ID or name and always return the ID.

    In non-VPC EB environments the configuration returns a security group name;
    in VPC environments it returns an ID directly. This helper handles both.
    """
    if identifier.startswith("sg-"):
        return identifier
    return get_security_group_by_name(identifier)['GroupId']


def _authorize_ingress_idempotent(client, group_id: str, **params) -> None:
    try:
        client.authorize_security_group_ingress(GroupId=group_id, **params)
    except client.exceptions.ClientError as e:
        if e.response['Error']['Code'] != 'InvalidPermission.Duplicate':
            raise


def _authorize_egress_idempotent(client, group_id: str, **params) -> None:
    try:
        client.authorize_security_group_egress(GroupId=group_id, **params)
    except Exception as e:
...
def allow_443_traffic_from_load_balancer_to_instances(eb_environment_name):
    ...
    instance_sec_grp_id = _sec_grp_id(get_eb_instance_security_group_identifier(eb_environment_name))
    ...
    log.info("creating ingress security group rule on EB instances from the load balancer")
    _authorize_ingress_idempotent(client, instance_sec_grp_id, **ingress_params)
    log.info("creating egress security group rule on load balancer to EB instances")
    _authorize_egress_idempotent(client, load_balancer_sec_grp_id, **egress_params)
...
def allow_eb_environment_database_access(eb_environment_name):
    ...
    eb_sec_grp_id = _sec_grp_id(get_eb_instance_security_group_identifier(eb_environment_name))
...
def create_eb_environment(eb_environment_name, without_db=False):
    ...
    # If the environment already exists (e.g. from a partial previous run that
    # failed after creation), skip re-creation and just apply the security group
    # rules, which are safe to call more than once.
    if check_if_eb_environment_exists(eb_environment_name):
        log.info("EB environment %s already exists — applying security group rules." % eb_environment_name)
